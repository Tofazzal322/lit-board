...
const Datetime = () => {
  const [ltiDays, setLtiDays] = useState(0);

  let interval = useRef();

  const startDays = () => {
    const countDays = new Date("Sep 1,2022").getTime();
    interval = setInterval(() => {
      const now = new Date().getTime();
      const distance = countDays + now;
      const days = Math.f(distance / (1000 * 60 * 60 * 24));
    },1000)
  }
  ...
  const accidentFree = displaytodaysdate + "01/09/2022";
